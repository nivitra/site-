...
import LanguageMarqueeSection from "@/components/language-marquee/LanguageMarqueeSection";
import IndustryUseCasesIso from "@/components/iso-features/IndustryUseCasesIso";
...
export async function generateMetadata({
  ...
  return {
    ...
    description: `${ind.tagline} Live in days, in 10 Indian languages, from ₹3.99/min.`,
...
export default async function IndustryPage({
  ...
  const jsonLd = {
    ...
    "@graph": [
      ...
      },
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: "Industry Intelligence", item: "https://speaksy.in/solutions" },
  ...
  return (
    <>
          ...
          <div className="flex flex-col items-start gap-5">
            <Reveal>
              <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-muted">
                <Link href="/solutions" className="transition-colors hover:text-brand-300">
            ...
            <Reveal delay={0.06}>
              <div className="flex flex-wrap items-center gap-3">
                <span className="inline-flex items-center gap-2 rounded-full border border-brand-500/30 bg-brand-500/10 px-4 py-1.5 text-xs font-semibold tracking-wide text-brand-700">
                  <span className="rounded bg-brand-500/15 px-1.5 py-0.5 font-mono text-[10px]">{ind.icon}</span>
                  {ind.name}
                </span>
                <span className="rounded-full border border-line px-4 py-1.5 text-xs font-medium text-muted">
                  {totalUseCases} Industry Intelligence flows
                </span>
              </div>
            </Reveal>
            <Reveal delay={0.12}>
      ...
      {/* use cases — isometric objects, per group */}
      <IndustryUseCasesIso
          ...
          <Reveal>
            <div className="card rounded-3xl p-8 sm:p-10">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-brand-500/30 bg-brand-500/10 text-xs font-semibold text-brand-700">
                  HR
                </span>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-3xl px-6">
          <SectionHeading
            eyebrow="FAQ"
            title={`${ind.name} voice AI, common questions`}
          />
      ...
      {/* Languages after industry content */}
      <LanguageMarqueeSection industryName={ind.name} />
        ...
        <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {others.map((i) => (
              ...
              >
                <span className="mr-1.5 rounded bg-brand-500/10 px-1.5 py-0.5 font-mono text-[10px] font-semibold text-brand-700">
                  {i.icon}
                </span>
                <span className="font-semibold text-brand-700">{i.name}</span>
              </Link>
            ))}
          </div>
        </div>
